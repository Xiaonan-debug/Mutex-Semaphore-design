...
/* Various atomic primitives for use in your own lock implementations */
...
// Create a semaphore, no need for condvar
int init(sem_t* sem, unsigned int size);

// Lock a semaphore, no need for condvar
// wait until the lock available
int wait(sem_t* sem);

// Try to Lock a semaphore, no need for condvar
// if a decrement could not block the thread returns an error
int try_wait(sem_t* sem);

// Unlock a semaphore, no need for condvar
int post(sem_t* sem);
