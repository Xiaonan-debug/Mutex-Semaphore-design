...
uint64_t find_my_thread() {
  ...
  for (uint64_t i = 0; i < (num_threads + THREAD_OFFSET); i++) {
    if (me == tid[i]) {
      return i;
    }
  }
...
static void reset_timer(uint64_t which) {
  ...
  if (timer_settime(timer[which], 0, &it, NULL)) {
...
static void interrupt_producer(uint64_t which) {
  ...
  int rc       = ring_try_push(ring, (void*)gen);
  if (rc < 0) {
...
static void interrupt_consumer(uint64_t which) {
  ...
  uint64_t gen;
  int rc = ring_try_pull(ring, (void**)&gen);
...
static void thread_producer(uint64_t which) {
  ...
  for (uint64_t i = 0; i < mypartterminals; i++) {
...
int main(int argc, char* argv[]) {
  ...
  uint32_t ring_size = atoi(argv[optind + 2]);
  ...
  if (interrupt_producers || interrupt_consumers) {
    ...
    if (sigaction(INTERRUPT_SIGNAL, &sa, NULL)) {
  ...
  for (uint64_t i = THREAD_OFFSET; i < (num_threads + THREAD_OFFSET); i++) {
    long rc = pthread_create(&(tid[i]), // thread id gets put here
  ...
  for (uint64_t i = THREAD_OFFSET; i < (num_threads + THREAD_OFFSET); i++) {
    ...
    long rc = pthread_join(tid[i], NULL);
    if (rc != 0) {
